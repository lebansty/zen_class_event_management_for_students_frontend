      ...
      
      {
        stuTable.map((val,idx)=>{
          return (
            ...
            <td>{val.batchName}</td>
            <td>
            <Link to={`/home/capstone/${val._id}`} type="button" className="btn btn-success mx-2 btn-sm">Capstone</Link>
            <Link to={`/home/webcode/${val._id}`} type="button" className="btn btn-success btn-sm">Webcode</Link>
            </td>
          </tr>
          )
        })
